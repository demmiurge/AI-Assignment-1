...
public class Evade : SteeringBehaviour
{
    ...
    {
        return Evade.GetLinearAcceleration(m_Context, target);
    }
    ...
    public static Vector3 GetLinearAcceleration(SteeringContext me, GameObject target)
    {
        ...
        float currentSpeed = targetContext.m_Velocity.magnitude;

        // determine the time it will take the target to reach me
        float predictedTimeToMe = distanceToMe / currentSpeed;
        if (predictedTimeToMe > me.m_MaxPredictionTime)
            predictedTimeToMe = me.m_MaxPredictionTime;

        // now determine future (at predicted time) location of target
        Vector3 futurePositionOfTarget = target.transform.position + targetContext.m_Velocity * predictedTimeToMe;
